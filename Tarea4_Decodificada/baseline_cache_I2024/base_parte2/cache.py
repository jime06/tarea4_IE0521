...
class cache:
    ...
    def access(self, access_type, access_address):
        ...
        cach_index = int(floor(access_address/(2**self.byte_offset_size))%(2**self.index_size))
        cache_tag = int(floor(access_address/(2**(self.byte_offset_size + self.index_size))))
        access_result = self.find(cach_index, cache_tag)
        access_hit = False 
        if access_result == -1:
            self.bring_to_cache(cach_index, cache_tag)
            self.total_misses += 1 
            if access_type == "r":
